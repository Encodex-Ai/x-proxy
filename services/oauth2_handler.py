...
class OAuth2Handler:
    ...
    def setup_oauth2_handler(self):
        os.environ["OAUTHLIB_INSECURE_TRANSPORT"] = (
            "1"  # Allow OAuth2 over HTTP for development
        )
    ...
    def initial_oauth2_setup(self):
        ...
        oauth2_authorization_url = input(
            "Please paste the authorization response URL here: "
        )

        self.oauth2_token = self.oauth2_user_handler.fetch_token(
            oauth2_authorization_url
        )
        self.oauth2_token["expires_at"] = (
            time.time() + self.oauth2_token["expires_in"] - 300
        )
        ...
        with self.refresh_lock:
            try:
                ...
                new_token = self.oauth2_user_handler.refresh_token(
                    self.oauth2_token["refresh_token"]
                )
                self.oauth2_token.update(new_token)
                self.oauth2_token["expires_at"] = (
                    time.time() + new_token["expires_in"] - 300
                )
                self.save_oauth2_token()
    ...
    def ensure_oauth2_token(self):
        ...
        if (
            self.oauth2_token.get("expires_at", 0) - current_time < 600
    ...
    def start_refresh_thread(self):
        def refresh_loop():
            while True:
                ...
                sleep_time = min(
                    time_to_expiry - 600, 3600
                )  # Sleep until 10 mins before expiry or for 1 hour, whichever is shorter
